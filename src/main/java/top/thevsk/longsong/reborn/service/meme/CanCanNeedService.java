...
@Service
public class CanCanNeedService extends BaseMemeService implements IMessageService {

    @Override
    public void groupMessage(GroupMessageEvent event, ApiSender sender) {
        if (!EventUtils.startsWith(event, "看看你的")) return;
        ...
        sender.sendGroupMsg(
                ...
                new Message()
                        .addMsg(reply(event))
                        .addMsg(tempImage(canCanNeedQQ(event.getUserId(), qq)))
        );
    }
}
